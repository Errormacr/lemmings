...
class App:
    ...
    def interaction(self): #метод, который обрабатывает пользовательский ввод и изменяет состояние игры в соответствии с ним. Он реагирует на нажатия клавиш, такие как стрелки для передвижения курсора, пробел для начала игры, и т.д. Также в этом методе создаются инструменты, которые игрок может использовать для помощи леммингам.
        # управление кнопками
    ...
    def disable_tool(self, tool_x, tool_y): #метод, который удаляет инструмент из списка инструментов на игровом поле.
    ...
    def update(self): #метод, который обновляет состояние игры на каждом шаге. В этом методе обновляется состояние леммингов, проверяется, прошел ли игрок уровень или нет, и обновляется интерфейс игры.
...
App()#конструктор класса приложения, который инициализирует объекты и переменные, необходимые для игры, и запускает цикл игры.
