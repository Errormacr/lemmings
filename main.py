...
class App:
    ...
    def __init__(self): #инициализация объектов, интерфейса, курсора игрока
       ...
       # лемминги
        ...
        pyxel.init(self.width, self.height, self.pyxel_window_title)
        ...
        for right_s in self.tools["right_s"]:
           ...
           #если правая лестница
            if right_s[0] == tool_x and right_s[1] == tool_y:
                self.tools["right_s"].remove([tool_x,tool_y,(0, 0, 0, 16, 16, 0), {'bottom_left': [tool_x, tool_y+16], 'top_right': [tool_x+16, tool_y]}])
        for left_s in self.tools["left_s"]:
            #если левая лестница
            if left_s[0] == tool_x and left_s[1] == tool_y:
                self.tools["left_s"].remove([tool_x,tool_y,(0, 0, 0, -16, 16, 0), {'bottom_left': [tool_x, tool_y], 'top_right': [tool_x, tool_y]}])
